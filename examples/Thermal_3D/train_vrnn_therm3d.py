...
normalization = SpectralNormalization(dim=3, features_min=features_min, features_max=features_max, bounds_fn=train_data.calc_bounds)
...
print(summary(model_norm, input_size=(batch_size, in_dim), dtypes=[torch.float32], device=device))
...
torch.save(model, data_dir / f'Thermal3D_models/vrnn_therm3D_{current_time}.pt')
fig.savefig(data_dir / f'Thermal3D_models/vrnn_therm3D_training_history_{current_time}.png', dpi=300)
