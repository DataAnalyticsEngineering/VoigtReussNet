...
from vrnn.normalization import NormalizedDataset, NormalizationModule, VanillaNormalization
from vrnn.data_mechanical import Dataset3DMechanical
from vrnn.models import VanillaModule, MixedActivationMLP
from vrnn.tensortools import unpack_sym, IsoProjectionC
...
# get only isotropic prjections
train_data.targets = torch.tensor(IsoProjectionC(train_data.targets))
val_data.targets = torch.tensor(IsoProjectionC(val_data.targets))
...
normalization = VanillaNormalization(dim=6, features_min=features_min, features_max=features_max)
...
torch.save(model, data_dir / f'Mechanical3D_models/isovann_mech3D_{current_time}.pt')
fig.savefig(data_dir / f'Mechanical3D_models/isovann_mech3D_training_history_{current_time}.png', dpi=300)
